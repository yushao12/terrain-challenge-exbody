...
"""
G1版本的motion retarget脚本
从CMU motion数据retarget到G1机器人
只处理下半身12个DOF的关节
"""
...
def project_joints_g1(motion):
    """G1版本的关节投影，只处理下半身关节"""
    # 获取G1的关节ID（基于12DOF配置）
    right_thigh_id = motion.skeleton_tree._node_indices["right_hip_pitch_joint"]
    right_shin_id = motion.skeleton_tree._node_indices["right_knee_joint"]
    right_foot_id = motion.skeleton_tree._node_indices["right_ankle_roll_joint"]
    left_thigh_id = motion.skeleton_tree._node_indices["left_hip_pitch_joint"]
    left_shin_id = motion.skeleton_tree._node_indices["left_knee_joint"]
    left_foot_id = motion.skeleton_tree._node_indices["left_ankle_roll_joint"]
...
def save_all():
    ...
    print(f"Processing {len(all_motion_names)} motions for G1...")

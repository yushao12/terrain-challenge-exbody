...
from legged_gym.envs.h1_2.h1_2_mimic import *
from legged_gym.envs.h1_2.h1_2_mimic_amp import H1_2MimicAMP
import os
from legged_gym import LEGGED_GYM_ROOT_DIR, ASE_DIR

class H1_2MimicViewMotion(H1_2MimicAMP):
                ...
                if len(save_ids) > 0:
                    for i in save_ids:
                        if not self.saved_flags[i]:
                            ...
                            # 保存到H1_2专用目录
                            np.save(os.path.join(ASE_DIR, f"ase/poselib/data/retarget_npy_h1_2/{self.motion_names[i]}_key_bodies.npy"), self.to_save_list[i].cpu().numpy())

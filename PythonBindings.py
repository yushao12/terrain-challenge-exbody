...
EXTRA_STATIC_LIBS      = []
EXTRA_LIBS             = []
SIZE_T_AS_ULONG        = None # This one is set ONLY if the Python interpreter is 3.4 or less and
                              # will tell sip compiler to typedef size_t as an 'unsigned long' instead of 'unsigned long long'
                              # which is the default that must be used when building x64

# link with FBX SDK library built with VisualStudio 2015 by default
vcCompiler = "vc140"
vsCompiler = "vs2015"

# vsPlafrom override the default detection that configure.py performs
# If you change it, make sure that configure.py support it!
vsPlatform = "win32-msvc%s" % (vsCompiler.replace('vs', ''))
...
def tag(platform_tag):
    ...
    if SIZE_T_AS_ULONG:
        # disable SIZE_T_AS_ULONGLONG feature so we define_sip_module
        # size_t as 'unsigned long' instead of 'unsigned long long'
        t = t + ' -x SIZE_T_AS_ULONGLONG'
...
def main(args):
    ...
    global EXTRA_LIBS
    global SIZE_T_AS_ULONG
    
    if WINDOWS_PLATFORM:
        if platform_tag == "FBX_X86":
            libPath = os.path.join(fbxsdkRootPath, 'lib', vsCompiler, 'x86', 'Release')
            if 'FBXSDK_LIBS_32_FOLDER' in os.environ:
                libPath = os.environ['FBXSDK_LIBS_32_FOLDER']
        elif platform_tag == "FBX_X64":
            libPath = os.path.join(fbxsdkRootPath, 'lib', vsCompiler, 'x64', 'Release')
    ...
    if buildSip:
        ...
        if int(pyVer[0]) < 3 or (int(pyVer[0]) == 3 and int(pyVer[1]) < 5):
            SIZE_T_AS_ULONG = True
    ...
    if platform.system() == 'Darwin':
        xtraflags = ['-framework', 'Carbon']
        if platform_tag == 'FBX_X86':
            xtraflags += ['-arch i386']
        elif platform_tag == 'FBX_X64':
            xtraflags += ['-arch x86_64']
        elif platform_tag == 'FBX_X86_X64':
            xtraflags += ['-arch i386', '-arch x86_64']
        makefile.extra_include_dirs = EXTRA_INCLUDE_DIRS_MAC
        makefile.extra_lflags = xtraflags
    else:
